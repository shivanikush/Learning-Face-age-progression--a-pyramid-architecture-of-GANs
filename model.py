...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.bn(x)
        x=self.relu(x)
        #print("size of residual",residual.shape)
...
def Path4(x):
    ...
    pretrained_dict = vgg16.state_dict()
    model_dict = model.state_dict()
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict.items()}
    model_dict.update(pretrained_dict)
    model.load_state_dict(model_dict)
    if torch.cuda.is_available():
        model = model.cuda()
...
def load_batch(batch_size=1, is_testing=False):
        ...
        path = glob('/home/shivani/ganpy1/GAN/CACD2000/*.jpg')
        print("path",path)
...
class GAN(object):
    def __init__(self, args):
        ...
        #summary(self.G,(3,224,224))
        #utils.print_network(self.D)
        print('-----------------------------------------------')

        self.sample_z = load_batch(1,False)
        print("z",self.sample_z)
        if self.gpu_mode:
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, (x, y) in enumerate(self.dataloader1):
                ...
                if ((iter + 1) % 100) == 0:
                    print('Epoch: [%2d] [%4d/%4d] D_loss: %.8f' %
                          (
                              (epoch + 1), (iter + 1), self.dataloader1.dataset.__len__() // self.batch_size,
                              D_loss.item()
                              
                          ))
                    #print("data",dataloader1)
            ...
            self.train_hist['per_epoch_time'].append(time.time() - epoch_start_time)8
